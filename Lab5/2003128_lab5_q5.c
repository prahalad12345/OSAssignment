...
{  pid_t pid;          /* variable to record process id of child */
   ...
   *in = *out = 0;          /* initial starting points */
   ...
   semid = sem_create(12);//creating 12 semaphore
   sem_init(semid, buf_used, 0);//on
   sem_init(semid, buf_space,12);
    int bufferid[5]={2,3,4,5,6};
    int bufferread[5]={7,8,9,10,11};
    for(int i=0;i<5;i++){
        sem_init(semid,bufferid[i],1);
        sem_init(semid,bufferread[i],0);
    }
       ...
       if (0 == pid)             
        { /* processing for parent == writer */
          ...
          for (i = 0; i < writer_length; i++)
             ...
             *in%= BUF_SIZE;
             P(semid,bufferid[id]);    /* wait semaphore for buffer access */
             buffer[id] = value;/* put data in buffer */
             
             //printf("write %d %d\n",*in,buffer[*in]);
             V(semid, bufferread[id]);    /* signal semaphore for buffer access */
         ...
         { /* processing for child == reader */
           ...
           for (i = 0; i < reader_length; i++)
             ...
             *out%= BUF_SIZE;
              ...
              P(semid, bufferread[id]);     /* wait semaphore for buffer access */
              value = buffer[id];/* take data from buffer */
              //printf("read %d %d\n",*out,buffer[*out]);
              V(semid, bufferid[id]);     /* signal semaphore for buffer access */
