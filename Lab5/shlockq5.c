...
{  pid_t pid;          /* variable to record process id of child */
   ...
   /* semaphore elements */
   int semid, mutex_id, in_out_lock;       /* identifier for a semaphore set */
   ...
   *in = *out = 0;          /* initial starting points */
   ...
   semid = sem_create(2);
   mutex_id = sem_create(BUF_SIZE);
   in_out_lock = sem_create(1);

   sem_init(semid, buf_used, 0);
   sem_init(semid, buf_space, BUF_SIZE);
   sem_init(in_out_lock, 0, 1);

   for (i=0; i<BUF_SIZE; i++) sem_init(mutex_id, i, 1);
       ...
       if (0 == pid)             
        { /* processing for parent == writer */
          ...
          for (i = 0; i < writer_length; i++)
          {
            ...
            // Wait for permission to write something.
            P(semid, buf_space);
            ...
            *in = (x+1) % BUF_SIZE;
            ...
            // Wait for permission to write something in that specific spot
            P(mutex_id, x);
            buffer[x] = value;
            V(mutex_id, x);
           ...
           for (i = 0; i < reader_length; i++)
            {
              ...
              *out = (x + 1) % BUF_SIZE;
              ...
              // Wait to read from that specific spot
              P(mutex_id, x);
              value = buffer[x];
              V(mutex_id, x);
              ...
              printf ("%2d || Reader %d: item %2d == %2d\n", value, p_count, i, value);
