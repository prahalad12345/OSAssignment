...
#define BUF_SIZE 7           /* logical size of buffer */
...
{  /* procedure to create and initialize semaphores and return semaphore id,
   ...
   if ((semid = semget (IPC_PRIVATE, 3, IPC_CREAT | 0600)) < 0)  /*https://man7.org/linux/man-pages/man2/semget.2.html*/
   ...
   if (semctl (semid, buf_space, SETVAL, 1) < 0) /*https://man7.org/linux/man-pages/man2/semctl.2.html*/
   ...
   if (semctl (semid, buf_used, SETVAL, 0) < 0) 
     ...
     if (semctl (semid, mutex, SETVAL, 1) < 0) /*https://man7.org/linux/man-pages/man2/semctl.2.html*/
     {  perror ("error in initializing first semaphore");
        exit (1);
     }
   return semid;
